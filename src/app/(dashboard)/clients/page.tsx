...
// ─── Mock client data ───
const mockClients = [
    { id: 1, name: "Tariq Mansour", phone: "+962791234567", email: "tariq@email.com", visits: 24, totalSpend: 312, rating: 5, lastVisit: "2 days ago", preferredBarber: "Ahmad", avatar: "TM" },
    { id: 2, name: "Sami Khalil", phone: "+962791234568", email: "sami@email.com", visits: 18, totalSpend: 245, rating: 4, lastVisit: "5 days ago", preferredBarber: "Khalid", avatar: "SK" },
    { id: 3, name: "Rami Abu-Said", phone: "+962791234569", email: "rami@email.com", visits: 31, totalSpend: 420, rating: 5, lastVisit: "1 day ago", preferredBarber: "Omar", avatar: "RA" },
    { id: 4, name: "Nabil Darwish", phone: "+962791234570", email: "nabil@email.com", visits: 12, totalSpend: 168, rating: 4, lastVisit: "1 week ago", preferredBarber: "Faris", avatar: "ND" },
    { id: 5, name: "Walid Khoury", phone: "+962791234571", email: "walid@email.com", visits: 8, totalSpend: 96, rating: 5, lastVisit: "3 days ago", preferredBarber: "Ahmad", avatar: "WK" },
    { id: 6, name: "Bassem Hani", phone: "+962791234572", email: "bassem@email.com", visits: 15, totalSpend: 198, rating: 4, lastVisit: "4 days ago", preferredBarber: "Yousef", avatar: "BH" },
    { id: 7, name: "Mazen Sabbagh", phone: "+962791234573", email: "mazen@email.com", visits: 42, totalSpend: 580, rating: 5, lastVisit: "Today", preferredBarber: "Omar", avatar: "MS" },
    { id: 8, name: "Fadi Nassar", phone: "+962791234574", email: "fadi@email.com", visits: 6, totalSpend: 78, rating: 3, lastVisit: "2 weeks ago", preferredBarber: "Khalid", avatar: "FN" },
];

const accentColors = ["var(--accent-mint)", "var(--accent-lavender)", "var(--accent-blue)", "var(--accent-amber)", "var(--accent-rose)"];
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-[20px] tracking-tight text-[var(--text-primary)] font-light">{t.sidebar.clients}</h2>
                        ...
                        >
                                ...
                                <div className="flex items-center gap-0.5">
                                    {[...Array(5)].map((_, si) => (
                                        <Star
                                            key={si}
                                            size={10}
                                            className={si < client.rating ? "text-[var(--accent-amber)]" : "text-[var(--text-muted)]"}
                            ...
                            <div className="grid grid-cols-3 gap-2 relative z-10">
                                <div className="text-center p-2 rounded-lg bg-[var(--bg-surface)]">
                                    <p className="text-[10px] text-[var(--text-muted)] font-light">{isRTL ? "زيارات" : "Visits"}</p>
                                    <p className="text-[14px] text-[var(--text-primary)] font-light">{client.visits}</p>
                                </div>
                                <div className="text-center p-2 rounded-lg bg-[var(--bg-surface)]">
                                    <p className="text-[10px] text-[var(--text-muted)] font-light">{isRTL ? "إنفاق" : "Spent"}</p>
                                    <p className="text-[14px] text-[var(--text-primary)] font-light">{client.totalSpend}</p>
                                </div>
                                <div className="text-center p-2 rounded-lg bg-[var(--bg-surface)]">
                                    <p className="text-[10px] text-[var(--text-muted)] font-light">{isRTL ? "الحلاق" : "Barber"}</p>
                                    <p className="text-[12px] text-[var(--text-primary)] font-light truncate">{client.preferredBarber}</p>
                                </div>
                            </div>
                        ...
                        >
                            <div className="p-6">
                                ...
                                <div className="text-center mb-6">
                                    ...
                                    <h4 className="text-[16px] text-[var(--text-primary)] font-light">{selectedClient.name}</h4>
                                    <div className="flex items-center justify-center gap-0.5 mt-1">
                                        {[...Array(5)].map((_, si) => (
                                            <Star
                                                key={si}
                                                size={12}
                                                className={si < selectedClient.rating ? "text-[var(--accent-amber)]" : "text-[var(--text-muted)]"}
                                                fill={si < selectedClient.rating ? "var(--accent-amber)" : "none"}
                                            />
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    {[
                                        { icon: Phone, label: t.leads.phone, value: selectedClient.phone },
                                        { icon: Mail, label: t.leads.email, value: selectedClient.email },
                                        { icon: Scissors, label: isRTL ? "الحلاق المفضل" : "Preferred Barber", value: selectedClient.preferredBarber },
                                        { icon: Calendar, label: isRTL ? "آخر زيارة" : "Last Visit", value: selectedClient.lastVisit },
                                        { icon: User, label: isRTL ? "إجمالي الزيارات" : "Total Visits", value: selectedClient.visits.toString() },
                                        { icon: DollarSign, label: isRTL ? "إجمالي الإنفاق" : "Total Spend", value: `${selectedClient.totalSpend} JOD` },
                                    ].map((field) => (
                                        <div key={field.label}>
                                            ...
                                            <div className="h-9 px-3 rounded-lg bg-[var(--bg-surface)] border border-[var(--border-primary)] flex items-center">
                                                <span className="text-[13px] text-[var(--text-primary)] font-light">{field.value}</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
