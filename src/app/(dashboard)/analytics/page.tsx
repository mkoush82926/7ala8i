...
const summaryMetrics = [
    { id: "revenue", icon: TrendingUp, value: "5,620", unit: "JOD", change: 12.5, color: "var(--accent-mint)" },
    { id: "bookings", icon: BarChart3, value: "172", unit: "", change: 8.3, color: "var(--accent-lavender)" },
    { id: "clients", icon: Users, value: "89", unit: "", change: 15.2, color: "var(--accent-blue)" },
    { id: "retention", icon: Star, value: "78%", unit: "", change: 3.1, color: "var(--accent-amber)" },
];
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h2 className="text-[20px] tracking-tight text-[var(--text-primary)] font-light">{t.sidebar.analytics}</h2>
                <div className="flex items-center gap-1 p-1 rounded-[var(--radius-sm)] bg-[var(--bg-surface)]">
                    {periods.map((p) => (
                        <button
                            key={p.key}
                            onClick={() => setPeriod(p.key)}
                            className={`px-4 py-1.5 rounded-[var(--radius-sm)] text-[12px] font-medium transition-all cursor-pointer ${period === p.key
                ...
                {summaryMetrics.map((m, i) => (
                    ...
                    >
                        ...
                        <p className="text-[32px] text-[var(--text-primary)] font-light tracking-tight relative z-10 mt-1">
                            {m.value} <span className="text-[14px] text-[var(--text-tertiary)]">{m.unit}</span>
                        </p>
                    </motion.div>
                ))}
            </div>
                    ...
                    <div className="h-[240px] relative z-10">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={revenueData}>
                                ...
                                />
                                <Area type="monotone" dataKey="revenue" stroke="#6EE7B7" strokeWidth={2} fill="url(#revGradient)" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </motion.div>
                    ...
                    <div className="h-[240px] relative z-10">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={barberData} layout="vertical" barSize={16}>
                                ...
                                <YAxis dataKey="name" type="category" tick={{ fill: '#999', fontSize: 11 }} axisLine={false} tickLine={false} width={60} />
                                <Tooltip
                                    contentStyle={{ background: '#111', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, fontSize: 12 }}
                                />
                                <Bar dataKey="revenue" radius={[0, 6, 6, 0]}>
                                    {barberData.map((_, i) => (
                                        <Cell key={i} fill={i === 0 ? "#6EE7B7" : i === 1 ? "#A78BFA" : i === 2 ? "#60A5FA" : "#444"} />
                                    ))}
                                </Bar>
                            </BarChart>
                        </ResponsiveContainer>
                ...
                >
                    ...
                    <div className="flex items-center gap-6 relative z-10">
                        <div className="w-[160px] h-[160px] flex-shrink-0">
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie data={acquisitionData} cx="50%" cy="50%" innerRadius={45} outerRadius={70} paddingAngle={3} dataKey="value">
                            ...
                            {acquisitionData.map((ch) => (
                                <div key={ch.name} className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[12px] text-[var(--text-secondary)] font-light">{ch.name}</span>
                                    </div>
                                    <span className="text-[12px] text-[var(--text-primary)] font-light tabular-nums">{ch.value}%</span>
                                </div>
                            ))}
                        </div>
                    </div>
                            ...
                            <thead>
                                <tr>
                                    ...
                                    {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
                                        <th key={d} className="text-[10px] text-[var(--text-muted)] font-light pb-2 text-center">{d}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {peakHours.map((row) => (
                                    <tr key={row.hour}>
                                        <td className="text-[10px] text-[var(--text-muted)] font-light py-0.5 pe-2">{row.hour}</td>
