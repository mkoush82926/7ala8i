    ...
    return (
        <motion.div
            ...
            variants={{
                ...
                visible: { opacity: 1, y: 0, transition: { duration: 0.7, delay, ease: [0.25, 1, 0.5, 1] } },
            }}
        >
            {children}
        </motion.div>
...
export default function LandingPage() {
    ...
    const features = [
        { icon: Calendar, title: t.landing.featureCalendar, desc: t.landing.featureCalendarDesc, color: "var(--accent-mint)" },
        { icon: Users, title: t.landing.featureCrm, desc: t.landing.featureCrmDesc, color: "var(--accent-lavender)" },
        { icon: Globe, title: t.landing.featureBooking, desc: t.landing.featureBookingDesc, color: "var(--accent-blue)" },
        { icon: BarChart3, title: t.landing.featureAnalytics, desc: t.landing.featureAnalyticsDesc, color: "var(--accent-amber)" },
        { icon: Star, title: t.landing.featureReviews, desc: t.landing.featureReviewsDesc, color: "var(--accent-rose)" },
        { icon: Smartphone, title: t.landing.featureMobile, desc: t.landing.featureMobileDesc, color: "var(--accent-mint)" },
    ];
                ...
                <div className="relative z-10 max-w-4xl mx-auto px-6 text-center pt-24">
                    ...
                    >
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-[var(--border-primary)] bg-[var(--bg-surface)] mb-8">
                            ...
                            <span className="text-[11px] text-[var(--text-tertiary)] font-light">{t.landing.trustedBy}</span>
                        </div>
                    </motion.div>
                            ...
                            <div className="rounded-xl bg-[var(--bg-secondary)] p-6">
                                ...
                                <div className="grid grid-cols-4 gap-3 mb-4">
                                    {[
                                        ...
                                        <div key={m.label} className="rounded-lg bg-[var(--bg-surface)] border border-[var(--border-primary)] p-3">
                                            <p className="text-[9px] text-[var(--text-muted)] uppercase tracking-wider">{m.label}</p>
                                            <p className="text-[20px] font-light mt-1" style={{ color: m.color }}>{m.val}</p>
                                        </div>
                                    ))}
                                </div>
                                <div className="h-24 rounded-lg bg-[var(--bg-surface)] border border-[var(--border-primary)] flex items-end p-3 gap-1">
                                    {[30, 45, 35, 55, 65, 50, 70, 85, 60, 75, 90, 80].map((h, i) => (
                                        <motion.div
                                            key={i}
                                            className="flex-1 rounded-sm"
                                            style={{ background: `linear-gradient(to top, var(--accent-mint), var(--accent-lavender))`, opacity: 0.6 }}
                                            initial={{ height: 0 }}
                                            animate={{ height: `${h}%` }}
                                            transition={{ duration: 0.6, delay: 0.8 + i * 0.05, ease: [0.25, 1, 0.5, 1] }}
                                        />
                                    ))}
                                </div>
                            </div>
                            ...
                            <Reveal key={i} delay={i * 0.15}>
                                <div className="text-center relative">
                                    <div className="w-14 h-14 rounded-2xl bg-[var(--bg-surface)] border border-[var(--border-primary)] flex items-center justify-center mx-auto mb-6 relative z-10">
                                        <span className="gradient-text text-[18px] font-medium">{step.num}</span>
                                    </div>
                        ...
                        <div className="glass-card-premium p-12 text-center">
                            <div className="mb-6 relative z-10">
                                <div className="flex items-baseline justify-center gap-1 mb-1">
                                    <span className="text-[48px] font-light gradient-text">1.5</span>
                                    <span className="text-[16px] text-[var(--text-secondary)] font-light">{t.common.jod}</span>
                                </div>
                            ...
                            <div className="space-y-3 mb-8 relative z-10">
                                {pricingFeatures.map((feat, i) => (
                                    <div key={i} className="flex items-center gap-3 justify-center">
                                        ...
                                        <span className="text-[13px] text-[var(--text-secondary)] font-light">{feat}</span>
                                    </div>
                                ))}
                            </div>
            ...
            <footer className="border-t border-[var(--border-primary)] py-16 px-6">
                <div className="max-w-6xl mx-auto">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-12">
                        ...
                        {/* Links */}
                        {[
                            { title: t.landing.product, links: [t.landing.featureCalendar, t.landing.featureCrm, t.landing.featureBooking, t.landing.featureAnalytics] },
                            { title: t.landing.company, links: [t.landing.about, t.landing.careers, t.landing.contact] },
                            { title: t.landing.legal, links: [t.landing.privacy, t.landing.terms] },
                        ].map((col) => (
                            <div key={col.title}>
                                <h4 className="text-[11px] text-[var(--text-muted)] uppercase tracking-wider mb-3">{col.title}</h4>
